...
class Oct3Dimage:
    ...
    def __str__(self):
        ...
        '''
        try:
            print('Stru data with shape （depth, width, frames）: ', self.stru3d.shape)
        except AttributeError:
            print('Stru data not found')

        try:
            print('Flow data with shape （depth, width, frames）: ', self.flow3d.shape)
        except AttributeError:
            print('Flow data not found')

        try:
            print('Segmentation data with shape （depth, width, layer_num）: ', self.layers.shape)
        except AttributeError:
            print('Seg data not found')

        output_info = 'print complete'
        return output_info
    """
# %%
#    def __del__(self):
...
#%% image subpixel registration
    ...
    '''
    def save_all_layer_volume(self, dir):
        """ Save all layers volumes
        """
        for i in range(0, self.layer_num-1):
            # save the stru layers
            if self.exist_stru:
                img_layer = self.save_layer_volume(self.stru3d, i)
                file_name = os.path.join(dir, 'layer_stru_'+str(i)+'.avi')
                self.save_video(img_layer, file_name)

            # save the flow layers
            if self.exist_flow:
                img_layer = self.save_layer_volume(self.flow3d, i)
                file_name = os.path.join(dir, 'layer_flow_'+str(i)+'.avi')
                self.save_video(img_layer, file_name)
    '''
