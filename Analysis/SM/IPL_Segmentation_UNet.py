...
def OAC_calculation_test(img):
    ...
    try:
        img_float = np.single(img) / 80
        OCTL = (20 ** (img_float - 1))
        maxim = np.empty_like(OCTL)
        #OCTL = ((10 ** (np.single(img.astype(float)) / 80) - 1) / 7.1)  # linear oct
        #maxim = integrate.cumulative_trapezoid(np.flipud(OCTL), axis=0)  # M size(H-1, W, frame)
        maxim[:-1, :, :] = integrate.cumulative_trapezoid(np.flipud(OCTL), axis=0)
        maxim[-1, :, :] = 0
        maxim = np.flipud(maxim)
        maxim = (maxim - np.min(maxim)) / (np.max(maxim) - np.min(maxim))
        eps = 1e-10
        JLin = OCTL[:-1, :, :] / (maxim[:-1, :, :] * 2 * (3 / img.shape[0]) + eps)  # pixel_size_z = 3mm/pixel_num_Z
...
def IPL_segmentation_UNet_ch(img_obj, paras):
    ...
    for i in range(Z):
        ...
        #io.imsave(result_path, np.uint8(img_current[:, :, i] * 255))
    ...
    channel_2 = layers[:, :, 2]
    channel_2 = median_filter(channel_2, size=2)
    channel_3 = layers[:, :, 3]
    channel_3 = median_filter(channel_3, size=2)
    ...
    oopl = gaussian_filter(layers[:, :, 0], sigma=2)
    cchoi = gaussian_filter(layers[:, :, 1], sigma=2)
...
#add= os.getcwd()
#img_obj = Oct3Dimage()
#url = "D:\\Zeiss ORL img\\P4607 20170823\\"
#files = os.path.join(url, "P4607_Angio (6mmx6mm)_8-23-2017_9-35-59_OD_sn22852_cube_z.img")
#img_obj.read_stru_data(files)
#print(":::         NOTICE:  running sample dataset         :::")
#paras = {'retina_lower_offset': 5, 'retina_upper_offset': 5, 'upper_bound': 5, 'lower_bound': 5}
#layers = IPL_segmentation_UNet_ch(img_obj,paras)
#np.save(os.path.join(url, "layers1.npy"), layers)
